...
/**
 * Command index used as wildcard for AddSubscription.
 */
static constexpr int MethodIndexForAddSubscription = 256;

/**
 * Command index used as wildcard for RemoveSubscription.
 */
static constexpr int MethodIndexForRemoveSubscription = MethodIndexForAddSubscription + 1;
        ...
        m_hyperlink(juce::HyperlinkButton(ProjectHostShortURL, juce::URL(ProjectHostLongURL)))
{
    ...
    m_ocaPropertyComboBox.onChange = [=]()
    {
        ...
        if (propIdx > 0)
        {
            ...
            m_ocaCommandComboBox.addItem("RemoveSubscription", MethodIndexForRemoveSubscription);
    ...
    m_ocaCommandComboBox.onChange = [=]()
    {
        ...
        if (commandIdx > 0)
        {
            ...
            // AddSubscription and RemoveSubscription commands selected.
            // In case of AddSubscription make m_ocaNotificationTextEditor visible.
            if ((m_ocaCommandComboBox.getSelectedId() == MethodIndexForAddSubscription) ||
                (m_ocaCommandComboBox.getSelectedId() == MethodIndexForRemoveSubscription))
            {
                m_ocaNotificationTextEditor.setVisible(m_ocaCommandComboBox.getSelectedId() == MethodIndexForAddSubscription);
...
bool StringGeneratorPage::CreateBinaryStrings(juce::MemoryBlock& commandMemBlock, juce::MemoryBlock& responseMemBlock, juce::MemoryBlock& notificationMemBlock)
{
    ...
    // Depending on whether the Get, Set, or Add/RemoveSubscription commands are selected,
    ...
    else if (methodIdx == MethodIndexForRemoveSubscription)
    {
        commandDefinition = NanoOcp1::Ocp1CommandDefinition(targetOno,
                                                            static_cast<std::uint16_t>(prop.m_type),
                                                            static_cast<std::uint16_t>(prop.m_defLevel),
                                                            static_cast<std::uint16_t>(prop.m_index)).RemoveSubscriptionCommand();
    }
